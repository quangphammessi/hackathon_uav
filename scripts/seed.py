...
TABLES_TO_RESET = [
    "trace_spans", "orders", "trust_ledger", "trust_tokens", "provenance_events",
                ...
                cur.execute(
                    """INSERT INTO products
                         (sku, gtin, name, category, description, attributes, currency,
                          internal_cost, map_price, list_price, inventory_units, batch, updated_at)
                       ...
                       ON CONFLICT (sku) DO UPDATE SET
                         ...
                         currency=EXCLUDED.currency, internal_cost=EXCLUDED.internal_cost,
...
def main() -> None:
    ...
    if not args.reembed:
        print(f"products : {seed_products()} upserted")
        print(f"provenance: {seed_provenance()} EPCIS events upserted")
        n_products, accepted, rejected = seed_features()
        print(f"features : {n_products} SKUs seeded; {accepted} competitor observations accepted")
        for r in rejected:
